#Add in and fix data
setwd("C:/Users/feinez/Desktop/WAE Maturation")
...
for (i in 1:length(unique(data$Year))) {
  plot(Maturity ~ TL.mm, data=data[data$Year==unique(data$Year)[i],], main=unique(data$Year)[i], ylim=c(0,2.5))
}
table(data$Year, data$Age)
data$Cohort <- data$Year - data$Age
table(data$Cohort, data$Maturity)
data$Mat <- data$Maturity - 1

head(data)
data$BinCohort <- as.character(cut(x=data$Cohort, breaks=seq(min(data$Cohort),max(data$Cohort),2), inlcude.lowest=TRUE))
data$FiveCohort <- as.character(cut(x=data$Cohort, breaks=seq(min(data$Cohort),max(data$Cohort),5), inlcude.lowest=TRUE))
data$FiveCohort[data$Cohort>1998] <- "(1998,2002]"
data$FiveCohort <- as.factor(data$FiveCohort)
data$BinCohort <- as.factor(data$BinCohort)
head(data)

#Fit lmer models to see what fits data best
library(lme4)
library(lsmeans)
str(data)
data <- data[!is.na(data$Month),]
data$Cohort <- as.factor(data$Cohort)
data$ObsAge <- data$Age

#Test for differences among months
data$Month <- as.factor(data$Month)

m.test <- glm(Mat ~ Month + Age * TL.mm, data=data[data$Age<7,], family="binomial")
summary(m.test)
lsmeans(m.test, ~Month|Age+TL.mm,transform='response')

plot(Mat ~ TL.mm, data=data[data$Age==3,])

plot(Mat ~ TL.mm, data=data[data$Age==4 & data$Month==9,])

data$Day <- lubridate::day(as.Date(data$Date,"%m/%d/%Y"))
plot(Mat ~ TL.mm, data=data[data$Age==4 & data$Month==5 & data$Day<=10,])

plot(Mat ~ TL.mm, data=data[data$Age==3 & data$Month==5 & data$Day>=15,])

summary(glm(Mat ~ Day + TL.mm,data=data[data$Age==3 & data$Month==5 & data$Day<=10,]))


#Exclude fish from after May 10 to September 1 (latest date of spawning across years to timing of repro investment)
#Adjust ages so that fish aged after May 10 have 1 added to age to represent spawning the following year
table(data$Day[data$Month==5])


data <- subset(data, Month %in% c(1,2,3,4,9,10,11,12) | (Month == 5 & Day <= 10))

#########Random effect intercept and slopes in stan with interaction and correlation matrix###########################################
table(data$Mat, data$Age)
for (i in 1:10) {
  plot(Maturity ~ TL.mm, data=data[data$Age==i,],main=i)
}

data$Age <- data$ObsAge
data$Age[data$Month %in% c(6,7,8,9,10,11,12,1,2)] <- data$ObsAge[data$Month %in% c(6,7,8,9,10,11,12,1,2)] + 1


set <- data[data$Age %in% c(2:7),]
#set <- set[c(seq(-2408,-2,2)),]
nrow(set)

set
set$Cohort <- as.numeric(factor(set$Cohort))
set$BinCohort <- as.numeric(factor(set$BinCohort))
set$FiveCohort <- as.numeric(factor(set$FiveCohort))
head(set)
str(set)
...
modlm = glmer(Mat ~ (1+Age*scale(TL.mm)|as.factor(Cohort))+Age*scale(TL.mm), family="binomial",control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e4)))
summary(modlm)

modlm.2 = glmer(Mat ~ (1+Age*scale(TL.mm)|as.factor(Cohort))+Age+scale(TL.mm), family="binomial",control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e4)))
#summary(modlm.2)

modlm.3 = glmer(Mat ~ (1+Age+scale(TL.mm)|as.factor(Cohort))+Age+scale(TL.mm), family="binomial",control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e4)))
#summary(modlm.3)

modlm.4 = glmer(Mat ~ (1 + scale(TL.mm)|as.factor(Cohort)) + (1+scale(TL.mm)|as.factor(Age)) + scale(TL.mm) * as.factor(Age), family="binomial",control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e4)))

BIC(modlm, modlm.2,modlm.3,modlm.4)

#summary(modlm)
#summary(modlm.2)

thing <- lm(TL.mm ~ log(Age),data=set)
#summary(thing)
thing2 <- lm(TL.mm ~ Age, data=set)
#summary(thing2)
thing3 <- lmer(TL.mm ~ (1+Age|Cohort) + Age, data=set)
summary(thing3)

BIC(thing,thing2,thing3)
...
fit = stan(model_code = stanmatcode, data=dat, init=inits, 
           iter=6000, warmup=3000, thin=12, chains=4, control=list(adapt_delta=0.95,max_treedepth=15))
...
plot(set$TL.mm[set$Cohort==10],set$Mat[set$Cohort==10],col=set$Age[set$Cohort==10])
...
plot(set$TL.mm[set$Cohort==25],set$Mat[set$Cohort==25],col=set$Age[set$Cohort==25])
...
plot(thing[thing$Cohort==6,1] ~ TL.mm, data=thing[thing$Cohort==6,],col=thing$Age[thing$Cohort==6],ylim=c(-0.5,1),xlim=c(300,800))
for (i in 2:1000) {
...
##
par(mfrow=c(1,1))
plot(Median ~ Year, data=sub.Lp75[sub.Lp75$Age==3,],type="b",col=1,pch=20,ylim=c(250,800),xlim=c(1950,2000),main="Age 3",cex=2,lwd=2,ylab="Median Lp75")
...
##
par(mfrow=c(1,1))
plot(Median ~ Year, data=sub.Lp50.all[sub.Lp50.all$Age==3,],type="b",col=1,pch=20,ylim=c(100,700),xlim=c(1950,2000),main="",cex=2,lwd=2,ylab="Median Lp50.all")
